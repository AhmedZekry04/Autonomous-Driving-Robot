...
class LaneDetector(object):
    ...
    def get_binary(self, image):
        # HSV colour space — more robust than LAB under yellow ambient lighting.
        # High saturation threshold (150) rejects yellow light (S~30-80) while
        # keeping yellow tape (S~180-255). Hue 20-35 covers yellow in OpenCV 0-179 scale.
        # Tune order: H range first, then raise S_min until ambient light vanishes.
        img_hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
        img_blur = cv2.GaussianBlur(img_hsv, (3, 3), 3)
        mask = cv2.inRange(img_blur, (10, 80, 50), (30, 255, 255))
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if image is not None:
            ...
            cv2.line(img, (min_x, y), (640, y), (255, 255, 255), 50)  # 画虚拟线来驱使转弯
